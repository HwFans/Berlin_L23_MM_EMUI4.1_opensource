...
static struct tps65133_platform_data *pdata;
...
static long tps65133_ioctl_do_ioctl(struct file *file, unsigned int cmd, void __user *arg, int compat_mode)
{
	...
	switch (cmd) {
		...
		case TPS65133_DISABLE:
			...
			break;
		default:
			pr_err("%s: invalid command %d", __func__, _IOC_NR(cmd));
			ret = -EINVAL;
			break;
	}
...
static int tps65133_probe(struct platform_device *pdev)
{
	...
	if (0 > ret) {
		pr_err("%s: set gpio direction failed", __func__);
	...
	if (ret) {
		pr_err("%s: tps65133_device register failed", __func__);
		goto gpio_hifi_pwr_err;
	}
